...
#..............basemap requires lat and lon to be in increasing order
    ...
    ticks=np.arange(lim1,lim2,step)
    ticks_label = [str(o) for o in ticks]
    #ticks6_label = ['1e-35','1e-32','1e-29','1e-26','1e-23','1e-20','1e-17','1e-14']
    #ticks6_label = ['1e-35','1e-30','1e-25','1e-20','1e-15']
    #ticks6_label = ['-1e2','1e0','1e1','1e2','1e3','1e4','1e5','1e6','1e7','1e8','1e9']
   ...
   # ticks6_label = ['-1e-22','1e-30','1e-16']
    ...
    x=plt.contourf(new_lon_final,lat,data_final,vmin=lim1,vmax=lim2,transform=ccrs.PlateCarree(),cmap='seismic_r',levels=ticks)
...
def get_model_data(mpath,data_level):
    ...
    ccn_file = '2017jan_m01s34i968_cloud_drop_number_conc.nc'
...
#for i in range(len(alt_data)):
    ...
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-bx903/2017jan/'  # pre-industrial model run
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-bx122/2017jan/'  # new model run
    ...
    if i%3==0 and i<49:
        ...
        temp_str='Change in CDNC at 1km - relative to pre-industrial'
        #c.data=np.where(c.data>=0,c.data,1e-20) #IMPORTANT LINE TO MASK UNNCESSARY DATA
        plot_diff(c,temp_str,-200,201,20)
