...
#..............basemap requires lat and lon to be in increasing order
   ...
   # ticks6_label = ['-1e-22','1e-30','1e-16']
    ...
    x=plt.contourf(new_lon_final,lat,data_final,vmin=-100,vmax=-30,transform=ccrs.PlateCarree(),cmap='viridis',levels=ticks6)
...
#for i in range(len(alt_data)):
    ...
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-by092/L1/'  # baseline model run
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-by352/L1/'  # new model run
    ...
    if i%3==0 and i<49:
        ...
        print('total soa = ', np.sum(c.data))
        temp_str='Percentage decrease in CCN (preindustrial)'
        #c.data=np.where(c.data>=0,c.data,1e-20) #IMPORTANT LINE TO MASK UNNCESSARY DATA
        plot_diff(c,temp_str,1e-20,1e0,20)
        plt.savefig('no_sulphur_ccn_2.eps',dpi = 500) 
