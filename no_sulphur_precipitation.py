...
#..............basemap requires lat and lon to be in increasing order
   ...
   # ticks6_label = ['-1e-22','1e-30','1e-16']
    ...
    x=plt.contourf(new_lon_final,lat,data_final,vmin=-100,vmax=100,transform=ccrs.PlateCarree(),cmap='seismic_r',levels=ticks6)
...
def get_model_data(mpath,data_level):
    ...
    ccn_file = 'All_months_m01s05i216_total_precipitation_rate.nc'
...
def get_model_data_2(mpath,data_level):
    ...
    model_n = ccn_data
    model_n=model_n.collapsed('longitude',iris.analysis.MEAN)
    #model_n=model_n.collapsed('model_level_number',iris.analysis.SUM) 
    #model_slice = model_n[12,:,:]
    model_slice = model_n.copy()
    #model_n = model_n*16*1e-12*(365*24*3600)
...
#for i in range(len(alt_data)):
    ...
    if i%3==0 and i<49:
        ...
        dat3=get_model_data_2(p_day1,data_level)
        dat4=get_model_data_2(p_day2,data_level)
        
        seasonal_perdiff = (dat4-dat3)*100.0/dat3
        prec_nh = seasonal_perdiff[:,72:] 
        prec_sh = seasonal_perdiff[:,0:72] 
        ...
        calc_mean_45 = c[35:108,:]
        print('mean value of pecenetage change in precipitation between 45N annd 45S = ', np.mean(calc_mean_45.data))
        print('global mean change in precipitation = ', np.mean(c.data))
        print('northern hemisphere  mean change in precipitation = ', np.mean(c[72:,:].data))
        print('southern hemisphere  mean change in precipitation = ', np.mean(c[:72,:].data))
        #c = b
        #c = a
        #c = b-a
        #d=dat3
        #e=(d-a)*100/a
        d = c.copy()
        #d.data = np.ma.masked_where(c.data>200,c.data) 
        d.data = np.where(c.data<100.0,c.data,200.0)
        #print('the highest is = ', np.max(d.data))
        #print('the lowest is = ', np.min(d.data))
         
        #print('mean = ', np.mean(d.data))
        temp_str='Percentage change in precipitation'
        #c.data=np.where(c.data>=0,c.data,1e-20) #IMPORTANT LINE TO MASK UNNCESSARY DATA
        plot_diff(d,temp_str,1e-20,1e0,20)
        plt.savefig('precipitation_nosulphur_2.eps',dpi = 500)
