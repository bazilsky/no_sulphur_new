...
mpath1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca206/All_months/' # with biogenic nucleation ON
mpath2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca179/All_months/'
 ...
 #       diff=((new1-old1)/old1)*100
      #  data, lat, lon = file_save(new1,i,alt_data[i])
       ...
       # ticks=[1e0,5e0,1e1,2e1,5e1,1e2,2e2,5e2,1e3,2e3,5e3,1e4,3e4,6e4,1e5]
        ...
        ticks4 = [1e0,1e1,5e1,1e2,2.5e2,5e2,7.5e2,1e3,2.5e3,5e3,7.5e3,1e4,2.5e4]
        ...
        plt.contourf(x,y,data,ticks4,norm=colors.LogNorm(vmin=np.min(ticks4),vmax=np.max(ticks4)),cmap=ruths_colors_r('roma'), format = '%.0e')
        ...
        plt.title('N_total change (cm'+u'\u207B\u00B3'+') (sulphurless+biogenic)-(sulphurless)' )
        #plt.title('Change in Particle number concentration (cm'+u'\u207B\u00B3'+')' )
