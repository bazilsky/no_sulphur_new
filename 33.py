...
#..............basemap requires lat and lon to be in increasing order
    ...
    ticks5 = [1e0,5e0,1e1,5e1,1e2,5e2,1e3,5e3]
    ticks6 = ['-1e0','-5e0','-1e1','-5e1','-1e2','-5e2','-1e3','-5e3']
    #x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap='RdYlBu',levels=ticks,vmin = -100, vmax = 100)
    #x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap='RdYlBu_r',levels=ticks5,vmin = 1e0, vmax = 5e3)
    x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap='RdYlBu_r',levels=ticks5,norm=colors.LogNorm(vmin=1e0,vmax=5e3))
...
#for i in range(len(alt_data)):
    ...
    if i%3==0 and i<49:
        ...
        temp_str='Percentage change in aitken-insol mode at 1km'
