...
#..............basemap requires lat and lon to be in increasing order
    ...
    ticks6 = [-1e-4,-1e-6,-1e-8,-1e-10,-1e-12,1e-12,1e-10,1e-8,1e-6,1e-4]
    ticks6 = [1e-8,2.5e-8,5e-8,7.5e-8,1e-7,2.5e-7,5e-7,7.5e-7,1e-6]
    ticks6 = [-100,-80,-60,-40,-20,0,20,40,60,80,100,500]
    ticks5 = [1e0,2.5e0,5e0,7.5e0,1e1,2.5e1,5e1,7.5e1,1e2,2.5e2]
    ticks5 = [1e0,5e1,1e2,5e2,1e3]
    ticks5 = np.arange(0,501,50)
    ticks5 = np.append(ticks5,[10200])
    ticks6 = [int(p) for p in ticks5]
    ticks6_label = [str(o)+'%' for o in ticks6]
    ticks6_label[len(ticks6)-1]='>500%'
    #ticks6_label[len(ticks6)-1] = '>100%'
    #ticks6_label = ['1e-35','1e-32','1e-29','1e-26','1e-23','1e-20','1e-17','1e-14']
    #ticks6_label = ['1e-35','1e-30','1e-25','1e-20','1e-15']
    #ticks6_label = ['-1e2','1e0','1e1','1e2','1e3','1e4','1e5','1e6','1e7','1e8','1e9']
   ...
   # ticks6_label = ['-1e-22','1e-30','1e-16']
    ...
    x=plt.contourf(new_lon_final,lat,data_final,ticks6,vmin=np.min(ticks6),vmax=500,transform=ccrs.PlateCarree(),cmap=ruths_colors_r('turku'))
    #x=plt.contourf(new_lon_final,lat,data_final,ticks6,norm=colors.LogNorm(vmin=np.min(ticks6),vmax=np.max(ticks6)),transform=ccrs.PlateCarree(),cmap=ruths_colors_r('turku'))
    #x=plt.contourf(new_lon_final,lat,data_final,ticks6,norm=colors.LogNorm(vmin=np.min(ticks6),vmax=np.max(ticks6)),transform=ccrs.PlateCarree(),cmap='RdYlBu_r')
    #x=plt.contourf(new_lon_final,lat,data_final,vmin=-100,vmax=100,transform=ccrs.PlateCarree(),cmap='RdYlBu_r',levels=ticks6)
    #x=plt.contourf(new_lon_final,lat,data_final,vmin=-100,vmax=100,transform=ccrs.PlateCarree(),cmap='RdYlBu_r',levels=ticks6)
    #x=plt.contourf(new_lon_final,lat,data_final,ticks6,norm=colors.SymLogNorm(linthresh = 1e-12,linscale = 1e-13,vmin=np.min(ticks6),vmax=np.max(ticks6)),transform=ccrs.PlateCarree(),cmap='seismic_r')
    #x=plt.contourf(new_lon_final,lat,data_final,vmin=lim1,vmax=lim2,transform=ccrs.PlateCarree(),cmap='seismic',levels=ticks6)
...
def get_model_data(mpath,data_level):
    ...
    #ccn_file = 'All_months_m01s00i254_QCL_AFTER_TIMESTEP__________________.nc'
    ccn_file = 'All_months_m01s38i525_unknown.nc' 
    ...
    model_slice = model_n[:,:]
    #model_slice = model_n[data_level,:,:]
...
#for i in range(len(alt_data)):
    ...
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-bz049/All_months/'  # baseline model run
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-bz050/All_months/'  # new model run
    ...
    if i%3==0 and i<49:
        ...
        #c = (b-a)*100.0/a
        c = b-a # sulphuerless - baseline sim 
        c = (b-a)*100.0/a
        #c = b
        #c = a
        #c = b-a
        ...
        temp_str='Percentage change in BC load'
        #c.data=np.where(c.data<100.0,c.data,200.0) #IMPORTANT LINE TO MASK UNNCESSARY DATA
        plot_diff(c,temp_str,-100,101,20)
