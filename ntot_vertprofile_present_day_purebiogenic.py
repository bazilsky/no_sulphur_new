...
mpath1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca250/All_months/' # with biogenic nucleation ON
mpath2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca123/All_months/'
 ...
 #       diff=((new1-old1)/old1)*100
      #  data, lat, lon = file_save(new1,i,alt_data[i])
       ...
       # ticks=[1e0,5e0,1e1,2e1,5e1,1e2,2e2,5e2,1e3,2e3,5e3,1e4,3e4,6e4,1e5]
        ...
        plt.contourf(x,y,data,ticks4,norm=colors.SymLogNorm(linthresh = 1e1,linscale = 1e0,vmin=-1e4,vmax=1e4),cmap='RdYlBu_r', format = '%.0e')
        ...
        print('\nThe mean value of particle number at 11.5km  = ', np.min(data),'\n')
        ...
        plt.title('Change in N_total (cm'+u'\u207B\u00B3'+') (present day + purebiogenic)-(presentday)' )
