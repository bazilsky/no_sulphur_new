...
#for i in range(len(alt_data)):
    ...
    #cn_file = 'All_months_m01s38i505_number_concentration_aitken_mode.nc'
    #cn_file = 'All_months_m01s38i506_number_concentration_accumulation_mode.nc' 
    #cn_file = 'All_months_m01s38i507_number_concentration_coarse_mode_insoluble.nc'
    cn_file = 'All_months_m01s38i508_number_concentration_aitken_insoluble.nc'
    #if i==0:
    if i%3==0:
 ...
 #       diff=((new1-old1)/old1)*100
      #  data, lat, lon = file_save(new1,i,alt_data[i])
       ...
       # ticks=[1e0,5e0,1e1,2e1,5e1,1e2,2e2,5e2,1e3,2e3,5e3,1e4,3e4,6e4,1e5]
        ...
        ticks4 = np.arange(20,201,20)
        ticks4_label = [str(o) for o in ticks4]
        flag2=ticks
        
        plt.contourf(x,y,data,ticks4,vmin=20,vmax=200,cmap='RdYlBu_r', format = '%.0e')
        ...
        plt.title('Change in aitken-insoluble mode (cm'+u'\u207B\u00B3'+')' )
        plt.savefig('presentation_1.eps',dpi = 500)
        plt.show()
