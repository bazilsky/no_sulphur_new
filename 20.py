...
mpath1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca976/All_months/' # with biogenic nucleation ON
mpath2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca439/All_months/'
...
#for i in range(len(alt_data)):
    ...
    # test nucleation mode particle concentration
    #cn_file = 'All_months_m01s38i504_number_concentration_nucleation_mode.nc'
    #if i==0:
    if i%3==0:
 ...
 #       diff=((new1-old1)/old1)*100
      #  data, lat, lon = file_save(new1,i,alt_data[i])
       ...
       # ticks=[1e0,5e0,1e1,2e1,5e1,1e2,2e2,5e2,1e3,2e3,5e3,1e4,3e4,6e4,1e5]
        ...
        ticks4 = [-5e4,-1e4,-5e3,-1e3,-5e2,-1e2,-5e1,-1e1,1e1,5e1,1e2,5e2,1e3,5e3,1e4,5e4] 
        ticks4 = [-1e4,-5e3,-1e3,-5e2,-1e2,-5e1,-1e1,1e1,5e1,1e2,5e2,1e3,5e3,1e4] 
        ticks4_label = [str(o) for o in ticks4]
        flag2=ticks
        
        plt.contourf(x,y,data,ticks4,norm=colors.SymLogNorm(linthresh = 1e1,linscale = 1e0,vmin=-5e3,vmax=1e4),cmap='RdYlBu_r', format = '%.0e')
        ...
        plt.title('Change in NTotal (cm'+u'\u207B\u00B3'+')-(PD + biogenic) - (PD) : u-ca976' )
        #plt.title('Change in Nuc mode conc (cm'+u'\u207B\u00B3'+')-(PD + biogenic) - (PD) ' )
