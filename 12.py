...
#..............basemap requires lat and lon to be in increasing order
   ...
   # ticks6_label = ['-1e-22','1e-30','1e-16']
    ...
    x=plt.contourf(new_lon_final,lat,data_final,vmin=np.min(ticks6),vmax=np.max(ticks6),transform=ccrs.PlateCarree(),cmap='RdYlBu_r',levels=ticks6)
...
#for i in range(len(alt_data)):
    ...
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-bz050/All_months/'  # baseline model run
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-bz693/All_months/'  # new model run
    ...
    if i%3==0 and i<49:
        ...
        temp_str='Percentage decrease in CDNC at 1km'
