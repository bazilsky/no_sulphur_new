...
#..............basemap requires lat and lon to be in increasing order
   ...
   # ticks6_label = ['-1e-22','1e-30','1e-16']
    ...
    x=plt.contourf(new_lon_final,lat,data_final,vmin=-100,vmax=0,transform=ccrs.PlateCarree(),cmap='RdGy',levels=ticks6)
...
def get_model_data(mpath,data_level):
    ...
    ccn_file = 'All_months_m01s34i081_mass_fraction_of_hydroxyl_radical_in_air.nc'
...
#for i in range(len(alt_data)):
    ...
    if i%3==0 and i<49:
        ...
        temp_str='Percentage increase in OH concentration at 1km - preindustrial'
        #c.data=np.where(c.data>=0,c.data,1e-20) #IMPORTANT LINE TO MASK UNNCESSARY DATA
        #plot_diff(c,temp_str,1e-20,1e0,20)
        pltfunc.plot_diff(c,temp_str,-40,40,'seismic_r')
        
        plt.savefig('oh_nosulphur_2.eps',dpi = 500) 
