...
#..............basemap requires lat and lon to be in increasing order
    ...
    #x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap='RdYlBu',levels=ticks,vmin = -100, vmax = 100)
    ticks4 = [1e0,5e0,1e1,5e1,-1e0,1e0,5e1,1e2,5e2,1e3]
    ticks4 = [-5e3,-1e3,-5e2,-1e2,-5e1,-1e1,1e1,5e1,1e2,5e2,1e3,5e3]
    #ticks4 = [1e0,5e0,1e1,5e1,1e2,5e2,1e3,5e3,1e4,5e4,1e5]
    #ticks5 = [-1e5,-5e4,-1e4,-5e3,-1e2,-5e1,-1e1,-5e0,-1e0]
    
    #x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap='RdYlBu_r',levels=ticks5,norm=colors.LogNorm(vmin=1e0,vmax=1e5))
    #x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap='RdYlBu_r',levels=ticks5,vmin=1e0,vmax=1e5))
    plt.contourf(new_lon_final,lat,data_final,ticks4,norm=colors.SymLogNorm(linthresh = 1e1,linscale = 1e0,vmin=-5e3,vmax=5e3),cmap='RdYlBu_r', format = '%.0e')
    #x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap='RdYlBu',levels=ticks5,norm=colors.LogNorm(vmin=1e5,vmax=1e0))
    ...
    ticks6 = ['-1e0','-5e0','-1e1','-5e1','-1e2','-5e2','-1e3','-5e3','-1e4','-5e4','-1e5']
    cbar=plt.colorbar(ax=ax,ticks=ticks4)
    
    #cbar.ax.tick_params(labelsize=16)
    cbar.ax.set_yticklabels(ticks4)
    cbar.ax.tick_params(labelsize=16)
...
data_level=35 # cloud base level = 12

for i in range(1):
#for i in range(len(alt_data)):
    print('\n File STARTED ')
    #if i==0:
    #p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-bx121/L1/' # this is the baseline
    #p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-bx122/L1/' # new run  

    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca439/L1/' # this is the baseline
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca557/L1/' # new run  
    cn_file = 'All_months_m01s38i437_CN__NUMBER_CONCENTRATION____________.nc'

    new1 = iris.load(p_day1[:-3]+'All_months/'+cn_file)[0]
    new2 = iris.load(p_day2[:-3]+'All_months/'+cn_file)[0]
    
    new1 = new1.collapsed('time',iris.analysis.MEAN)
    new2 = new2.collapsed('time',iris.analysis.MEAN)
    
    new1 = new1[data_level,:,:]
    new2 = new2[data_level,:,:]
    
    if cn_file != 'All_months_m01s38i437_CN__NUMBER_CONCENTRATION____________.nc':
        new1 = new1/1.0e6
        new2 = new2/1.0e6
    ...
    if i%3==0 and i<49:
        ...
        temp_str='Change in N_total at 10km (PD+ biogenic) - (PD)'
        plot_diff(c,temp_str,-100,101,10)
        print('max = ', np.max(c)) 
        print('min = ', np.min(c))
