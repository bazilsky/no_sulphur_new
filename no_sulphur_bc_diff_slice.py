...
#..............basemap requires lat and lon to be in increasing order
   ...
   # ticks6_label = ['-1e-22','1e-30','1e-16']
    ...
    x=plt.contourf(new_lon_final,lat,data_final,ticks6,norm=colors.LogNorm(vmin=1e-8,vmax=1e-6),transform=ccrs.PlateCarree(),cmap=ruths_colors_r('turku'))
    #x=plt.contourf(new_lon_final,lat,data_final,ticks6,norm=colors.LogNorm(vmin=1e-8,vmax=1e-6),transform=ccrs.PlateCarree(),cmap='RdYlBu_r')
...
#for i in range(len(alt_data)):
    ...
    if i%3==0 and i<49:
        ...
        temp_str='Change in BC load (kg/m2)'
