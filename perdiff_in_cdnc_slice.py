...
#..............basemap requires lat and lon to be in increasing order
   ...
   # ticks6_label = ['-1e-22','1e-30','1e-16']
    color_path = '/home/users/eeara/ScientificColourMaps6/davos/davos.txt' 
    cm_data_2 = np.loadtxt(color_path)
    cm_data = np.flip(cm_data_2,axis=0)
    color_map = LinearSegmentedColormap.from_list("davos", cm_data)
    ...
    x=plt.contourf(new_lon_final,lat,data_final,vmin=-100,vmax=0,transform=ccrs.PlateCarree(),cmap=color_map,levels=ticks6)
...
#for i in range(len(alt_data)):
    ...
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca123/All_months/'  # baseline model run
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca179/All_months/'  # new model run
    
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca439/All_months/'  # baseline model run
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca440/All_months/'  # new model run
    ...
    if i%3==0 and i<49:
        ...
        temp_str='Percentage decrease in CDNC at 1km (sulphurless - presntday)- NEW'
