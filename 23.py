...
#..............basemap requires lat and lon to be in increasing order
    ...
    x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap=ruths_colors_r('roma'),levels=ticks,vmin = lim1, vmax = lim2)
    #x=plt.contourf(new_lon_final,lat,data_final,transform=ccrs.PlateCarree(),cmap=ruths_colors('vik'),levels=ticks,vmin = lim1, vmax = lim2)
...
#--------------------------------
def get_cloud_flag(path,filename,level):
...
data_level=11 # cloud base level = 1.1km
#data_level=19 # cloud base level = 3km
#data_level=28 # cloud base level = 6km
#data_level=35 # cloud base level = 9km
...
#for i in range(len(alt_data)):
    ...
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca123/All_months/' # this is the baseline
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca250/All_months/' # new run  
    
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca439/All_months/' # this is the baseline
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca440/All_months/' # new run  
    
    
    p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca763/All_months/' # this is the baseline
    p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca765/All_months/' # new run  
    
    
    #p_day1='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca123/All_months/' # this is the baseline
    #p_day2='/group_workspaces/jasmin2/asci/eeara/model_runs/u-ca179/All_months/' # new run  
    if i%3==0 and i<49:
        
        #dat1=get_model_data(p_day1,data_level)
        #dat2=get_model_data(p_day2,data_level)
        new_cn_file = 'All_months_m01s38i479_TOTAL_CDNC_*_CLOUD_FLAG_(m^-3)______.nc'
        cn_file = 'All_months_m01s38i437_CN__NUMBER_CONCENTRATION____________.nc'
        cn_file = 'All_months_m01s34i968_cloud_drop_number_conc.nc' # units of m3
        cloud_flag_file = 'All_months_m01s38i478_CLOUD_FLAG__________________________.nc'
        #cn_file = new_cn_file
        ...
        cloud_flag_a = get_cloud_flag(p_day1,cloud_flag_file,data_level)
        cloud_flag_b = get_cloud_flag(p_day2,cloud_flag_file,data_level)
        
        b_new = b/cloud_flag_b
        a_new = a/cloud_flag_a

        #c=(b-a)*100/a
        #d=dat3
        #e=(d-a)*100/a
        c= (b-a)/1.0e6
        c_new = (b_new - a_new)/1e6 

        del_cloudflag = cloud_flag_b - cloud_flag_a

        print ('the highest is = ', np.max(c.data))
        print ('the lowest is = ', np.min(c.data))
        
        temp_str='Change in CDNC at 1km (cm'+u'\u207B\u00B3'+') (sulphurless - presentday)- NEW'
        temp_str2 = 'Change in cloud flag at 1km (sulphurless - present day)'
        plot_diff(c,temp_str,-200,1,20) # for 3km
        plot_diff(del_cloudflag,temp_str2,-1,1.1,0.2) # for 3km
        plot_diff(c_new,'change in cdnc_by_cloudflag (sulphurless - presentday)',-500,1,50) # for 3km
        print ('max = ', np.max(c)) 
        print ('min = ', np.min(c))
