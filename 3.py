...
#for i in range(len(alt_data)):
 ...
 #       diff=((new1-old1)/old1)*100
      #  data, lat, lon = file_save(new1,i,alt_data[i])
        ...
        #diff_2 = (new1.data-new2.data)*100.0/new2.data # biogenic - baseline simulation
        #diff = np.where(diff_2<100.0,diff_2,100.0) 
        #data=new1.data
        data = diff.data
       ...
       # ticks=[1e0,5e0,1e1,2e1,5e1,1e2,2e2,5e2,1e3,2e3,5e3,1e4,3e4,6e4,1e5]
        ...
        ticks5 = [-1e5,-5e4,-1e4,-5e3,-1e3,-5e2,-1e2,-5e1,-1e1,1e1,1e2,1e3,1e4,1e5]
        ticks5 = [-1e5,-1e4,-1e3,-1e2,-1e1,1e1,1e2,1e3,1e4,1e5]
        #ticks5 = [-1e2,-9.5e1,-9e1,-8.5e1,-8e1,-6e1,-4e1,-2e1,0]
        #ticks5 = [-100,-80,-60,-40,-20,0-5e4,-1e4,-5e3,-1e3,-5e2,-1e2,-5e1,-1e1,1e1]
        #ticks5 = np.arange(-110.0,1,10)
        #ticks5 = [-1e2,1e2]
        ...
        plt.contourf(x,y,data,ticks5,norm=colors.SymLogNorm(linthresh = 1e-12,linscale = 1e-13,vmin=-1e5,vmax=1e5),cmap='RdYlBu_r', format = '%.0e')
